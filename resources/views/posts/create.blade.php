...
<form class="flex flex-col w-1/2 mx-auto mt-5" class="w-full" action="{{ route('posts.store') }}" method="POST">
    @csrf
    @method('POST')
    ...
    <input
        ...
        id="title" name="title" type="text" placeholder="Post Title" value="{{ old('title')}}">
    <textarea
        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline my-2"
        name="body" id="body" cols="30" rows="10" placeholder="New post here">{{ old('body')}}</textarea>
